...
import { Gavel, AlertTriangle, CheckCircle2, XCircle, Download, RotateCcw, Scale, Loader2 } from "lucide-react";
...
export default function VerdictPage({ params }: { params: { id: string } }) {
  ...
  const [datasetName, setDatasetName] = useState("COMPAS");
  const [juryPersonas, setJuryPersonas] = useState(JURY_PERSONAS);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const storedDatasetName = localStorage.getItem("trialDatasetName") || "COMPAS";
        setDatasetName(storedDatasetName);
        
        const storedJury = localStorage.getItem("trialJury");
        if (storedJury) {
          setJuryPersonas(JSON.parse(storedJury));
        }

        const storedAnalysis = localStorage.getItem("trialAnalysis");
        
        let analyzePromise;
        if (storedAnalysis) {
          const analyzeData = JSON.parse(storedAnalysis);
          if (analyzeData.fairness_metrics) {
            setFairnessMetrics({
              demographic_parity: analyzeData.fairness_metrics.demographic_parity || analyzeData.fairness_metrics.demographicParity,
              equal_opportunity: analyzeData.fairness_metrics.equal_opportunity || analyzeData.fairness_metrics.equalOpportunity,
              disparate_impact: analyzeData.fairness_metrics.disparate_impact || analyzeData.fairness_metrics.disparateImpact,
            });
          }
          ...
          analyzePromise = Promise.resolve();
        } else {
          analyzePromise = fetch("/api/analyze", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ dataset_name: storedDatasetName }),
          }).then(async (res) => {
            if (res.ok) {
              const analyzeData = await res.json();
              if (analyzeData.fairness_metrics) setFairnessMetrics(analyzeData.fairness_metrics);
              if (analyzeData.shap_values) {
                setShapData(
                  analyzeData.shap_values
                    .map((sv: any) => ({
        ...
        const mitigatePromise = fetch("/api/mitigate", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ dataset_name: storedDatasetName, technique: "reweighting" }),
        }).then(async (res) => {
          if (res.ok) {
            const mitigateData = await res.json();
        ...
        await Promise.all([analyzePromise, mitigatePromise]);
  ...
  const generatePDF = () => {
    ...
    const shapRows = shapData.map(s => [
      ...
      s.feature.includes("(Proxy)") ? "YES" : "NO"
    ]);
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(11);
    const deniedCount = juryPersonas.filter(p => p.outcome === "Denied").length;
    doc.text(`${deniedCount} of ${juryPersonas.length} personas adversely affected by model bias.`, 14, yPos);
    yPos += 5;

    const juryRows = juryPersonas.map(p => [
      p.name,
      p.demographic,
      p.occupation,
      p.outcome
        ...
        >
            ...
            <div className="bg-surface border border-border rounded-xl p-2 space-y-2">
              ...
              <div className="p-4 bg-background border border-border rounded-lg flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-foreground/60 text-sm">Protected groups are significantly less likely to receive a favorable outcome compared to the majority group.</p>
                </div>
              ...
              <div className="p-4 bg-background border border-border rounded-lg flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-foreground/60 text-sm">Ratio of outcomes between protected and unprotected groups falls below the 80% legal threshold.</p>
                </div>
          ...
          <motion.section variants={FADE_UP_ANIMATION_VARIANTS}>
            ...
            <p className="text-lg font-medium text-red-600 bg-red-50 p-4 rounded-lg border border-red-100 mb-6 flex items-center gap-2">
              ...
              {juryPersonas.filter(p => p.outcome === "Denied").length} of {juryPersonas.length} jury members were adversely affected by model bias
            </p>
            ...
            <div className="bg-background border-2 border-border shadow-sm rounded-xl p-8 font-serif relative overflow-hidden">
              ...
              <ul className="space-y-4 font-sans">
                ...
                <li className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-bold">Remove highly correlated proxy features.</p>
                    <p className="text-sm text-foreground/60">Feature drop attributes that act as proxies for protected demographic classes.</p>
                  </div>
                </li>
                <li className="flex items-start gap-3">
